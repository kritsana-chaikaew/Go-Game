...
public class PanelRenderer {
    ...
    public void renderTroopLayer(TroopBlock troopBlock) {
        ...
        if (troopBlock.hasLayer(Troop.SWORDMAN)) {
            ...
            drawFont("" + panel.currentSwordMan, row + 5, column,
                28, 60, Color.BLACK);
            drawFont("" + panel.availableSwordMan, row + 5, column,
                28, 20, Color.GREEN);
        } else if (troopBlock.hasLayer(Troop.BOWMAN)) {
            drawBlock(Assets.trainImage, row + 5, column);
            drawBlock(Assets.bowImage, row, column);
            drawFont("" + panel.currentBowMan, row + 5, column,
                28, 60, Color.BLACK);
            drawFont("" + panel.availableBowMan, row + 5, column,
                28, 20, Color.GREEN);
        } else if (troopBlock.hasLayer(Troop.GUARDIAN)) {
            drawBlock(Assets.trainImage, row + 5, column);
            drawBlock(Assets.shieldImage, row, column);
            drawFont("" + panel.currentGuardian, row + 5, column,
                28, 60, Color.BLACK);
            drawFont("" + panel.availableGuardian, row + 5, column,
                28, 20, Color.GREEN);
        }

        if ( !troopBlock.hasLayer(Troop.EMPTY_TROOP) ) {
            if (troopBlock.getHP() > 9) {
                drawFont("" + troopBlock.getHP(), row, column,
                    24, 38, Color.RED);
            } else {
                drawFont("" + troopBlock.getHP(), row, column,
                    28, 38, Color.RED);
    ...
    public void renderEndTurnButton() {
        ...
        if (World.gameState == GameState.BLACK_TURN
                && panel.getStoneLayer() == Stone.BLACK
